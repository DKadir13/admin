...
Backend **http://localhost:3001** adresinde çalışmalı. Konsolda "MongoDB bağlandı" ve "Backend http://localhost:3001" gör.
...
- `VITE_API_BASE`: Admin backend’in adresi (yerelde **mutlaka** `http://localhost:3001`).
...
| 2 | `admin/backend` → `npm run dev` | API http://localhost:3001’de |
| 3 | Admin panelinden blog/ürün ekle | Veri MongoDB’ye yazılır |
| 4 | Diğer sitede `.env` → `VITE_API_BASE=http://localhost:3001` | İstek doğru adrese gider |
...
- **404:** Diğer sitede `VITE_API_BASE` boş veya yanlış. Mutlaka `http://localhost:3001` yaz, fetch’te `${API_BASE}/api/public/sites/...` kullan (relative `/api/...` kullanma).
- **CORS hatası:** Backend’te CORS açık (`cors({ origin: true })`). Backend’i yeniden başlat.
- **Boş liste:** Backend çalışıyor mu kontrol et: tarayıcıda http://localhost:3001/api/public/sites/atakentEczadeposu/blog aç; JSON (array) dönmeli. Admin’de ilgili siteden en az bir blog/ürün ekle ve durumu “yayında” / “aktif” yap.
