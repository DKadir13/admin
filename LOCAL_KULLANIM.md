...
Backend yerelde çalışmalı. Konsolda "MongoDB bağlandı" gör.
...
- `VITE_API_BASE_URL`: Frontend’in backend’e istek atacağı base path. Nginx mimarisinde **`/api`** kullanılır.
...
| 2 | `admin/backend` → `npm run dev` | API Nginx/proxy üzerinden `/api` ile erişilebilir |
| 3 | Admin panelinden blog/ürün ekle | Veri MongoDB’ye yazılır |
| 4 | Diğer sitede `.env` → `VITE_API_BASE_URL=/api` | İstek Nginx üzerinden backend’e gider |
...
- **404:** Diğer sitede `VITE_API_BASE_URL` yanlışsa `/api` proxy çalışmaz. Fetch’te **relative** `'/api/public/sites/...'` (veya `\`\${VITE_API_BASE_URL}/public/sites/...\``) kullanın; tam URL/port kullanmayın.
- **CORS hatası:** Backend’te CORS açık (`cors({ origin: true })`). Backend’i yeniden başlat.
- **Boş liste:** Backend çalışıyor mu kontrol et: tarayıcıda `/api/public/sites/atakentEczadeposu/blog` aç; JSON (array) dönmeli. Admin’de ilgili siteden en az bir blog/ürün ekle ve durumu “yayında” / “aktif” yap.
